...
def getStatistic(slope):
    ...
    valid = ~np.isnan( arry )
    arryNeg =  arry[ valid & ( arry < 0 ) ]
    arryNone = arry[ valid & ( arry == 0 ) ]
    arryPos =  arry[ valid & ( arry > 0 ) ]
    return {
        'neg': getValues( arryNeg ),
        'none': getValues( arryNone ),
        'pos': getValues( arryPos )
    }
...
def run(dir_tendences):
    ...
    total_statistic = getStatistic( slopes[0] ) # { neg: { min, sum, count, max }, none:{...}, pos: {...}
    slopes[0]['ds'] = None
